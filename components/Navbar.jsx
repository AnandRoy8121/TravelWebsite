...
const Navbar = ({ companyName }) => {
  ...
  return (
    <div
      className={`${
        scrolled
          ? "fixed top-0 flex justify-between w-full items-center bg-blue-600 z-40 shadow-md rounded-md"
          : "absolute top-0 flex justify-between w-full items-center bg-transparent"
      }`}
    >
      <Link href={"/"}>
        <h1 className="text-white font-bold mx-10 text-2xl z-40 cursor-pointer py-4">
          {companyName}
        </h1>
      </Link>

<div className="flex justify-center items-center md:hidden sm:block">

      <BiMenuAltRight onClick={handleNav} size={30} className='text-white z-40 cursor-pointer'/>
      <div className={nav ?'fixed top-0 left-0 z-30 text-gray-300 w-full h-screen bg-transparent/80 px-4 py-7 flex-col ease-out duration-700':'absolute z-10 top-0 left-[-100%] duration-1000 ease-in h-screen'}>
      ...
      <div className="sm:flex sm:justify-center sm:items-center mx-auto hidden">
